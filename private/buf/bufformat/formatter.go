...
func newFormatter(buffer *bytes.Buffer, fileNode *ast.FileNode) *formatter {
	return &formatter{
		buffer:   buffer,
		fileNode: fileNode,
	}
...
// Run runs the formatter and writes the file's content to the formatter's buffer.
...
func (f *formatter) P(elements ...interface{}) {
	if len(elements) > 0 {
		...
		fmt.Fprint(f.buffer, strings.Repeat("  ", f.indent))
	}
	for _, elem := range elements {
		fmt.Fprint(f.buffer, elem)
	}
...
// writeFileTypes writes the types defined in a .proto file. This includes the messages,
// enums, services, etc. All other elements are ignored since they are handled in the
// prelude.
func (f *formatter) writeFileTypes(fileNode *ast.FileNode) error {
	...
	for _, fileElement := range f.fileNode.Decls {
		...
		case *ast.PackageNode, *ast.OptionNode, *ast.ImportNode:
			// These elements have already been written in the prelude.
			continue
		default:
			if writeNewline {
...
func (f *formatter) writeMessageElements(messageElements *messageElements) error {
	// header groups all of the options, reserved, and extension ranges
...
func (f *formatter) writeEnumElements(enumElements *enumElements) error {
	// header groups all of the options and reserved ranges
	// in a single slice so that they are formatted together.
...
// are handled with simpler string conversions.
